...
> **Note.** Use the same file for [Getting the project's settings](#getting-the-projects-settings) step.
 ...
 * @param {String} [options.name='bem'] — Config filename. `rc` is appended to the filename, and config traverses files with this name with any extension (for example `.bemrc`, `.bemrc.js`, `.bemrc.json`).
 ...
 * @param {String} [options.pathToConfig] — Custom path to config in FS via the `--config` command line argument.
...
Sets the custom path to config in file system via the `--config` command line argument.
...
Returns an array of level configs for the level set.
